...
try {
  ...
  const status =
    gzippedSizeInKB <= VERCEL_EDGE_LIMIT
      ? `✅ você ainda tem ${(VERCEL_EDGE_LIMIT - gzippedSizeInKB).toFixed(2)} KB de sobra`
      : `❌ Passou o limite por ${(gzippedSizeInKB - VERCEL_EDGE_LIMIT).toFixed(2)} KB`
...
try {
  ...
  execSync(`bun run esbuild-analyzer --metafile=${targetPath}`, {
    stdio: 'inherit', //faz com que a saída apareça no console
  })
} catch (error) {
  console.error('❌ Falha no esbuild-analyzer:', error)
} finally {
  //finally garante que sempre sejá executado, mesmo em caso de erro
